...
class MapCenterPatient extends React.Component {
    state = {
        isMapInit: false,
        isDriver: false,
        taskData:null,
    ...
    componentWillMount() {
        this.setState({isDriver: this.props.isDriver, taskData: this.props.taskData})
    }
    ...
    render() {
        ...
        return (
            this.props.ready ? (
                <Map className='mapP1' center={PatientPos} zoom={this.props.zoom}
                     ref={this.saveMap}>
                    <TileLayer
                    ...
                    {
                        this.state.isDriver &&
                        <Marker key={[this.props.DriverPos.lat, this.props.DriverPos.lng]}
                                position={[this.props.DriverPos.lat, this.props.DriverPos.lng]} icon={redIcon}>
                    ...
                    <Marker key={PatientPos} position={PatientPos} icon={blueIcon}>
                        <Cercle map={this.map} pos={PatientPos}
                                color={"blue"} fillColor={"blue"} fillOpacity={0.1} radius={100}/>
                        <Popup>
                            <div className='poup-text'>Your Position</div>
                        </Popup>
                    </Marker>
...
export default connect(mapStateToProps, actions)(MapCenterPatient);
